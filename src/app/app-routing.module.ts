...
const routes: Routes = [
  ...
  },
  {
    ...
    children: [
      {
        path: 'add-order',
        loadChildren: () => import('./order/add-order/add-order.module').then(m => m.AddOrderPageModule)
      },
      {
        path: 'order-produk',
        loadChildren: () => import('./order/order-produk/order-produk.module').then(m => m.OrderProdukPageModule)
      }
    ]
  }
];
